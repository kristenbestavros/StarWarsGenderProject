pub mod gendercounting{
    ...
    pub fn percentwomen(g: &Graph,map: &HashMap<String,String>)->f32{
        let chars=&g.nodes;
    ...
    pub fn percentwomennodroids(g: &Graph, map: &HashMap<String,String>)->f32{
        let chars=&g.nodes;
        let mut numchars=chars.len() as f32;
        let mut numwomen: f32=0.0;
        for node in chars{
            match map.get(&node.name){
                Some(s)=>match s.as_str(){
                    "f"=>numwomen=numwomen+1.0,
                    "d"=>numchars=numchars-1.0,
                    _d=>(),
                },
                None=>(),
            }
    ...
    pub fn percentffinteractionswomen(g:&Graph,map:&HashMap<String,String>)->f32{
        let links=&g.links;
        let nodes=&g.nodes;
    ...
    pub fn percentffinteractionsall(g:&Graph,map:&HashMap<String,String>)->f32{
        let links=&g.links;
        let nodes=&g.nodes;
        ...
        let percent=ffinteractions/allinteractions;
        return percent
    }

    pub fn percentmminteractionsmen(g: &Graph,map: &HashMap<String,String>)->f32{
        let links=&g.links;
        let nodes=&g.nodes;
        ...
        for link in links{
            ...
            match tstuple{
                (Some(s),Some(t))=>match (s.as_str(),t.as_str()){
                    ...
                    ("m",_x)=>meninteractions=meninteractions+link.value as f32,
                    (_f,"m")=>meninteractions=meninteractions+link.value as f32,
                    (_f,_g)=>(),
                },
                (None,Some(_x))=>(),
                (Some(_y),None)=>(),
        ...
        let percent=mminteractions/meninteractions;
        return percent
    }

    pub fn percentmminteractionsall(g:&Graph,map:&HashMap<String,String>)->f32{
        let links=&g.links;
        let nodes=&g.nodes;
        let mut allinteractions:f32=0.0;
        let mut ffinteractions:f32=0.0;
